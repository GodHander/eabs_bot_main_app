...
public class AsrManagerImpl implements AsrManager {
    ...
    private EduInterface.AsrCallBack asrCallBack;
                                ...
                                if (name.equals(SpeechConstant.CALLBACK_EVENT_ASR_PARTIAL)) {
                                    ...
                                    if (!TextUtils.isEmpty(params)) {
                                        ...
                                        if (recogResult.isFinalResult()) {
                                            final String s = results[0];
                                            Log.e(TAG,"S---->"+s);
                                            if (asrCallBack != null) {
                                                asrCallBack.onGetAsrSuccess(s);
                                               AsrParserService.getInstance().parserWord(s, new TranslationWordCallBack() {
                                                   @Override
                                                   public void TranslationResult(TranslationWordBean response) {
                                                       asrCallBack.onGetAnswer(s,response);
                                                   }
                                                   @Override
                                                   public void TranslationFail(String failReason) {
                                                       asrCallBack.onGetAnswerFail(s, failReason);
                                                   }
                                               });
                                            }
                                        }
                                    ...
                                    if (asrCallBack != null)
                                        asrCallBack.onGetAsrFailed("null");
                                }
                            } else {
                                Log.e(TAG, "语音识别ASR" + params);
    ...
    @Override
    public void rigsterAsrListener(EduInterface.AsrCallBack callBack) {
